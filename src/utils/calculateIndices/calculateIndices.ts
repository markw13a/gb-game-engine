 ...
 */
export const calculateIndices = (subSectionSideLength: number, originalArraySideLength: number, position: number) => {
    if (subSectionSideLength > originalArraySideLength) {
        throw new Error(`Cannot take subsection of size ${subSectionSideLength} x ${subSectionSideLength} from ${originalArraySideLength} x ${originalArraySideLength} grid`);
    }
    ...
    // I I I
    // * * *
    const columnIndices = range.map(i => position + i * originalArraySideLength);
